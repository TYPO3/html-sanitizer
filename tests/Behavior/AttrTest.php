...
{
    public static function matchesNameDataProvider()
    {
        return [
     ...
     */
    public function matchesName($flags, $name, $matchName, $expectation)
    {
        $attr = new Attr($name, $flags);
        self::assertSame($expectation, $attr->matchesName($matchName));
    }

    public static function matchesValueDataProvider()
    {
     ...
     */
    public function matchesValue($flags, array $values, $matchValue, $expectation)
    {
        $attr = new Attr('test', $flags);
        call_user_func_array([$attr, 'addValues'], $values);
